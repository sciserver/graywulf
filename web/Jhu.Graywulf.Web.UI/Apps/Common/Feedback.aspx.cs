...
namespace Jhu.Graywulf.Web.UI.Apps.Common
{
    public partial class Feedback : PageBase
    {
        ...
        {
            return GetUrl(mode, Guid.Empty);
        }

        private static string GetUrl(Mode mode, Guid jobGuid)
        {
            var pars = "";
            var url = "~/Apps/Common/Feedback.aspx";

            if (mode != Mode.Unknown)
            {
                pars += String.Format("&mode={0}", mode);
            }

            if (jobGuid != Guid.Empty)
            {
                pars += String.Format("&guid={0}", jobGuid);
            }

            if (pars != "")
            {
                url += "?" + pars.Substring(1);
            }
        ...
        {
            return GetUrl(Mode.SpaceRequest);
        }

        public static string GetErrorReportUrl()
        {
            return GetUrl(Mode.Error);
        }

        public static string GetJobErrorUrl(Guid jobGuid)
        {
            return GetUrl(Mode.JobError, jobGuid);
        }
