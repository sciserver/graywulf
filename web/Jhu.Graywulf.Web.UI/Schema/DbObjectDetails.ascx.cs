...
namespace Jhu.Graywulf.Web.UI.Schema
{
    public partial class DbObjectDetails : UserControlBase
    {
        ...
        {
            Response.Redirect(String.Format("~/mydb/ExportTable.aspx?objid={0}", databaseObjectID));
        }

        protected void Rename_Click(object sender, EventArgs e)
        {
            Response.Redirect(String.Format("~/mydb/RenameObject.aspx?objid={0}", databaseObjectID));
        }

        protected void Drop_Click(object sender, EventArgs e)
        {
            Response.Redirect(String.Format("~/mydb/DropObject.aspx?objid={0}", databaseObjectID));
        }
    }
}